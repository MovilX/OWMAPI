...
class GetWeatherListUseCaseTest {
    ...
    fun testProcess() = runTest {
        ...
        whenever(repo.getWeatherList()).thenReturn(flowOf(listOf(weather1, weather2)))
        val response = useCase.process(GetWeatherListUseCase.Request).first()

        assertEquals(GetWeatherListUseCase.Response(listOf(weather1, weather2)), response)
    }
}